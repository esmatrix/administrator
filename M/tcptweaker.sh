...
then
	...
	echo ""
	echo "Las configuraciones de rede TCP Tweaker já foram adicionadas no sistema!"
	echo ""
	read -p "Deseja remover as configuraciones do TCP Tweaker? [s/n]: " -e -i n resposta0
	if [[ "$resposta0" = 's' ]]; then
		grep -v "^#PH56
...
sysctl -p /etc/sysctl.conf > /dev/null
		echo ""
		echo "As configuraciones de rede do TCP Tweaker foram removidas com sucesso."
		echo ""
	exit
	else 
		echo ""
